...
// src/core/index.ts
import fs2 from "fs";
import { createFilter, normalizePath as normalizePath3 } from "vite";
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  if (options.devServer) {
    ...
    if (filename.startsWith(options.root)) {
      assetUrlOptions = {
        base: (((_b = options.devServer.config.server) == null ? void 0 : _b.origin) ?? "") + devBase + slash(path.relative(options.root, path.dirname(filename)))
      };
    }
  } else {
    assetUrlOptions = {
...
import { normalizePath as normalizePath2, transformWithEsbuild } from "vite";
...
function createDescriptor(filename, source, { root, isProduction, sourceMap, compiler }) {
  ...
  const normalizedPath = slash(path2.normalize(path2.relative(root, filename)));
...
async function transformMain(code, filename, options, pluginContext, ssr, asCustomElement) {
  ...
  if (devToolsEnabled || devServer && !isProduction) {
    attachedProps.push([
      `__file`,
      JSON.stringify(isProduction ? path3.basename(filename) : filename)
    ]);
  }
  ...
  if (ssr) {
    const normalizedFilename = normalizePath2(
      path3.relative(options.root, filename)
    );
    output.push(
...
async function genScriptCode(descriptor, options, pluginContext, ssr) {
  ...
  if (script) {
    ...
    if ((!script.lang || script.lang === "ts" && inlineTs) && !script.src) {
      ...
      const langFallback = script.src && path3.extname(src).slice(1) || "js";
...
var core_default = createUnplugin((rawOptions = {}, meta) => {
  ...
  return {
    ...
    resolveId(id) {
      if (normalizePath3(id) === EXPORT_HELPER_ID) {
        return id;
      }
